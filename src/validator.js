const validator = {
  ...
  isValid: function () {
    ...
    if (ccn.length === 16) { // se revirtió el valor de ccn usando el condicional if, tomando en cuenta que length representa la longitud de una cadena.
      ccn = ccn.reverse();
  ...
  maskify: function () {
    const ccn = document.getElementById("cardnumber").value;

    console.log(ccn.length);
    let masked = "";
    if (ccn.length > 4) {
      ...
      masked = document.getElementById("cardnumber").value;
    }
